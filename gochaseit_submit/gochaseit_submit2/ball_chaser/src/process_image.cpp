...
void process_image_callback(const sensor_msgs::Image img)
{
	...
	//int grid[row][col];	//store pixel data in this matrix
	//int index;		//define index

	//ROS_INFO("image size- ht:%1.2f,step:%1.2f", (float)img.height, (float)img.step);
	...
	for(int i=0; i< img.height*img.step; i++){
		...
		//ROS_INFO("image dets- data:%1.2f, i: %i", (float)img.data[i], (int)(i%img.step));
		//if the pixel is white, find out where it is
			if((i%img.step) < (img.step/3)){
				    drive_robot(0,0.1);
				    break;}
				else if(((i%img.step) > (img.step/3)) && ((i%img.step)<(2*img.step/3)))			{drive_robot(0.2,0);	   
	    				break;}
